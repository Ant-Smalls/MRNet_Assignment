...
Usage:
    python3 explainability_multiplane.py \
        ...
        --output_html acl_multiplane_triage.html
"""
...
def parse_args():
    ...
    parser.add_argument('--output_html', type=str, default='multiplane_triage.html')
...
def find_most_informative_slice(model, exam, device):
    ...
    with torch.no_grad():
        ...
        # Find most frequently selected slice
        most_informative_slice = int(mode(slice_indices.cpu().numpy(), axis=1, keepdims=False)[0][0])
        contribution_count = int((slice_indices == most_informative_slice).sum().item())
        contribution_pct = (contribution_count / 256) * 100
